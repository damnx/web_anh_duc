...
class Use_admin extends MY_Controller
{
    ...
    public function index($parameter = NULL)
    {
        ...
        {
            $data['active'] = 'users_admin_trash';
        }
        else
        {
            $data['active'] = 'users_admin';
        }
        ...
        if ($post)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    $post['created'] = time();
                    $data['return'] = $this->use_model->insert($post,'admin');
                }
        ...
        if ($parameter == 'trash')
        {
            ...
            $this->load->view('backend/use/trash',isset($data) ? $data : Null);
            $this->load->view('backend/footer',isset($data) ? $data : Null);
        }
        else
        {
            $this->load->view('backend/header',isset($data) ? $data : Null);
            $this->load->view('backend/use/iteam',isset($data) ? $data : Null);
            $this->load->view('backend/footer',isset($data) ? $data : Null);
        }
    ...
    public function ajax($action = null)
    {
        ...
        else
        {
            ...
            if ($action == 'trash')
            {
                ...
                if ($id > 0)
                {
                    $data = $this->use_model->delete('admin',array('id' => $id));
