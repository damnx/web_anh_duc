...
class Attribute_model extends CI_Model {
    ...
    public function get_join_attribute($candidate_key = '')
    {
        $this->db->select('category.name');
        $this->db->from('category');
        $this->db->join('relationships', 'category.id = relationships.foreign_key');
        $this->db->join('attribute', 'attribute.id = relationships.candidate_key');
        $this->db->where('relationships.candidate_table', 'attribute');
        $this->db->where('relationships.candidate_key', $candidate_key);
        $query = $this->db->get()->result_array();
        return $query;
    }
    public function attributes($foreign_key = '')
    {
        $this->db->select('attribute.id, attribute.name, relationships.candidate_table, relationships.candidate_key,relationships.foreign_table,relationships.foreign_key');
        $this->db->from('attribute');
        $this->db->join('relationships', 'attribute.id = relationships.candidate_key');
        $this->db->where('relationships.candidate_table', 'attribute');
        $this->db->where('relationships.foreign_table', 'category');
        $this->db->where('relationships.foreign_key', $foreign_key);
