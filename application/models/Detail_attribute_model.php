...
class Detail_attribute_model extends CI_Model {
    ...
    public function detail_attribute($id_pr = 0,$id_att= 0 )
    {
        $this->db->select('*');
        $this->db->from('product_detail_attribute');
        $this->db->join('detail_attribute', 'product_detail_attribute.id_da = detail_attribute.id');
        $this->db->where(array('product_detail_attribute.id_product'=>$id_pr,'detail_attribute.id_attribute'=>$id_att));
        $query = $this->db->get()->result_array();
        return $query;
    }
}
