...
class Pratt_model extends CI_Model {
    ...
    public function get_join_type($candidate_key = '')
    {
        $this->db->select('category.name');
        $this->db->from('category');
        $this->db->join('relationships', 'category.id = relationships.foreign_key');
        $this->db->join('type', 'type.id = relationships.candidate_key');
        $this->db->where('relationships.candidate_table', 'type');
        $this->db->where('relationships.candidate_key', $candidate_key);
        $query = $this->db->get()->result_array();
        return $query;
    }
    public function attribute ($id_pr = 0)
    {
        $this->db->select('COUNT(product_detail_attribute.id),attribute.name,product_detail_attribute.id_att');
        $this->db->from('product_detail_attribute');
        $this->db->join('attribute', 'product_detail_attribute.id_att = attribute.id');
        $this->db->where(array('id_product'=>$id_pr));
        $this->db->group_by("id_att");
        $query = $this->db->get()->result_array();
        return $query;
    }
}
